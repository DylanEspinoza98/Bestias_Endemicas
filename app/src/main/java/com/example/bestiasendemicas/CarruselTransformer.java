package com.example.bestiasendemicas;

import android.view.View;
import androidx.annotation.NonNull;
import androidx.viewpager2.widget.ViewPager2;

public class CarruselTransformer implements ViewPager2.PageTransformer {

    private static final float MIN_SCALE = 0.85f; // Escala mínima para las páginas de los lados
    private static final float MIN_ALPHA = 0.5f;  // Opacidad mínima para las páginas de los lados


    /** Método que se llama cada vez que una página se desplaza en el ViewPager2*/
    @Override
    public void transformPage(@NonNull View page, float position) {
        int pageWidth = page.getWidth();
        int pageHeight = page.getHeight();

        if (position < -1) { // [-Infinity,-1)
            // Esta página está fuera de la pantalla a la izquierda.
            page.setAlpha(0f);
        } else if (position <= 1) { // [-1,1]
            // Modificar la escala por defecto y la transición de deslizamiento
            float scaleFactor = Math.max(MIN_SCALE, 1 - Math.abs(position));
            float vertMargin = pageHeight * (1 - scaleFactor) / 2;
            float horzMargin = pageWidth * (1 - scaleFactor) / 2;

            if (position < 0) {
                page.setTranslationX(horzMargin - vertMargin / 2);
            } else {
                page.setTranslationX(-horzMargin + vertMargin / 2);
            }

            // Escalar la página hacia abajo (entre MIN_SCALE y 1)
            page.setScaleX(scaleFactor);
            page.setScaleY(scaleFactor);

            // Desvanecer la página según su tamaño.
            page.setAlpha(MIN_ALPHA + (scaleFactor - MIN_SCALE) / (1 - MIN_SCALE) * (1 - MIN_ALPHA));
        } else { // (1,+Infinity]
            // Esta página está fuera de la pantalla a la derecha.
            page.setAlpha(0f);
        }
    }
}
